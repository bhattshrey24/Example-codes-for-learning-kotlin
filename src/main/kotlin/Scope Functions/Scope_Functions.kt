 ...
 class Scope_Functions {
    fun execute(){
        ...
        val lambdaRes= with(ob){
            ...
            this.age=22
            this.name="B"
            "Successfully changed" // this will be returned cause it's the last statement to "lambdaRes"
        }
       val modifiedOb = ob.apply {
        ...
        var lambdaRes2 = newOb?.run {
            // this function is mixture of 'with' and 'let' i.e. returning the lambda result with null safety
    ...
    class A(age:Int,name:String){// using data class would be more logical here
